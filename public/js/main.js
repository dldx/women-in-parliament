...
function second_slide(no_transition = false) {
    ...
    svg.transition()
            ...
            electionRects
                .on("mouseover", function (d, i) {
                    ...
                    if (chartTitle.text()
                        ...
                        <div class="gender-ratio">${gender_ratio.toFixed(1)}</div> <span class="male">men</span> Labour MPs.` :
        "There were no Labour women in the House of Commons :("}
                        ...
                        <div class="gender-ratio">${gender_ratio.toFixed(1)}</div> <span class="male">men</span> Conservative MPs.` :
        "There were no Conservative women in the House of Commons :("}
                        ...
                        <div class="gender-ratio">${gender_ratio.toFixed(1)}</div> <span class="male">men</span> Lib Dem or SNP MPs.` :
...
function fifth_slide(no_transition = false) {
    ...
    // Change credits
    credit_alink
        .attr("xlink:href", "https://www.theyworkforyou.com/")
        .select("text")
        .transition()
        .text("Data: TheyWorkForYou (raw transcripts)")
...
function update_fifth_slide(no_transition, default_selected_topic, from_scroll, drawMedian) {
    ...
    window.slide5_show_mp_tooltip = function (nodeData, mousePos) {
        ...
        tooltip.innerHTML = `
                    ...
                    "<img class=\"mp-image\" src=\"./assets/mp-images/mp-" + nodeData.id + ".jpg\" style=\"opacity: ${typeof nodeData.loaded == 'undefined' ? 0 : nodeData.loaded;d.loaded = 1;};\" onload=\"this.style.opacity = 1;\" />"}
                    </div>
                    <div class="body-facts">
            <div class="mp-partyname">${party_abbreviations[nodeData.party] || nodeData.party}</div>
                    <div class="mp-constituency">${nodeData.constituency}</div>
...
function download_data() {
    d3.queue()
        .defer(d3.csv, "data/women_mps.csv", function (d) {
    ...
    d3.queue()
        .defer(d3.csv, "data/mp_base64.csv", function (d) {
            return {
                id: d.id,
                base64: d.base64
            }
    ...
    d3.queue()
        ...
        .defer(d3.csv, "data/number_women_parliamentary_candidates_over_time.csv",
            d => {
                return {
                    year: d.Year,
                    Labour: +d.Lab_women,
        ...
        .defer(d3.csv, "data/photo_attribution.csv")
