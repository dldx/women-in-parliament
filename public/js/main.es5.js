...
function show_mp_tooltip(nodeData, mousePos) {
    ...
    if (typeof mp_base64_data == "undefined") {
        tooltip_innerHTML += "<img class=\"mp-image-blurred\" style=\"opacity: 0;\"/>\n                <img class=\"mp-image\" src=\"./assets/mp-images/mp-" + nodeData.id + ".jpg\" />\n                ";
    } else {
        // If mp has a photo
        if (typeof mp_base64_data[nodeData.id] !== "undefined") {
            tooltip_innerHTML += "<img class=\"mp-image-blurred\" src=\"data:image/jpeg;base64, " + mp_base64_data[nodeData.id] + "\"/>\n                <img class=\"mp-image\" src=\"./assets/mp-images/mp-" + nodeData.id + ".jpg\" style=\"opacity: " + (typeof nodeData.loaded == "undefined" ? 0 : nodeData.loaded) + (nodeData.loaded = 1) + ";\" onload=\"this.style.opacity = 1;\" />\n                ";
        }
    }
...
function second_slide() {
    ...
    // add the line path
    total_women_mps_path = slide2Group.append("path").attr("class", "total-women-mps-path slide2").datum(number_women_over_time_data).attr("stroke-width", 1.5 * lineThickness).attr("d", total_women_mps_line).style("opacity", 0);
        ...
        electionRects.on("mouseover", function (d, i) {
            ...
            if (chartTitle.text().includes("Labour")) {
                ...
                var num_women = Math.max.apply(Math, _toConsumableArray(number_women_over_time_data.filter(function (d) {
                    ...
                    return d.labour_women_mps;
                })));
                var gender_ratio = d.labour_mps / num_women - 1;
                tooltip.innerHTML = "<div class=\"slide2-tooltip\"><h1>" + formatDate(first_election) + " &rarr; " + formatDate(second_election) + "</h1>\n                        " + (num_women > 0 ? "<p>" + num_women + " Wom" + (num_women == 1 ? "a" : "e") + "n</p><hr/>\n                        For every <span class=\"female\">woman</span> Labour MP, there " + (new Date() > second_election ? "were" : "are") + "\n                        <div class=\"gender-ratio\">" + gender_ratio.toFixed(1) + "</div> <span class=\"male\">men</span> Labour MPs." : "There were no Labour women in the House of Commons :(") + "\n                        </div>\n                        ";
                ...
                num_women = Math.max.apply(Math, _toConsumableArray(number_women_over_time_data.filter(function (d) {
                    ...
                    return d.conservative_women_mps;
                })));
                gender_ratio = d.conservative_mps / num_women - 1;
                tooltip.innerHTML = "<div class=\"slide2-tooltip\"><h1>" + formatDate(first_election) + " &rarr; " + formatDate(second_election) + "</h1>\n                        " + (num_women > 0 ? "<p>" + num_women + " Wom" + (num_women == 1 ? "a" : "e") + "n</p><hr/>\n                        For every <span class=\"female\">woman</span> Conservative MP, there " + (new Date() > second_election ? "were" : "are") + "\n                        <div class=\"gender-ratio\">" + gender_ratio.toFixed(1) + "</div> <span class=\"male\">men</span> Conservative MPs." : "There were no Conservative women in the House of Commons :(") + "\n                        </div>\n                        ";
            } else if (chartTitle.text().includes("Liberal")) {
                num_women = Math.max.apply(Math, _toConsumableArray(number_women_over_time_data.filter(function (d) {
                    return d.year > first_election && d.year <= second_election;
                }).map(function (d) {
                    return d.lib_snp_women_mps;
                })));
                gender_ratio = d.lib_snp_mps / num_women - 1;
                tooltip.innerHTML = "<div class=\"slide2-tooltip\"><h1>" + formatDate(first_election) + " &rarr; " + formatDate(second_election) + "</h1>\n                        " + (num_women > 0 ? "<p>" + num_women + " Wom" + (num_women == 1 ? "a" : "e") + "n</p><hr/>\n                        For every <span class=\"female\">woman</span> Lib Dem or SNP MP, there " + (new Date() > second_election ? "were" : "are") + "\n                        <div class=\"gender-ratio\">" + gender_ratio.toFixed(1) + "</div> <span class=\"male\">men</span> Lib Dem or SNP MPs." : "There were no Lib Dem or SNP women in the House of Commons yet :(") + "\n                        </div>\n                        ";
            } else {
                num_women = Math.max.apply(Math, _toConsumableArray(number_women_over_time_data.filter(function (d) {
                    return d.year > first_election && d.year <= second_election;
                }).map(function (d) {
...
function third_slide() {
    ...
    function mouseover(d) {
        ...
        if (country_on_screen.indexOf(d.data.country) > -1) {
            ...
            tooltip.innerHTML = "\n                            <div class=\"slide3-tooltip\">\n                                <h1 style=\"border-color: " + (d.data.country == "United Kingdom" ? colors["Hover"] : countryColors(d.data.country)) + ";\">" + d.data.country + "</h1>\n                                For every <span class=\"female\">woman</span> MP, there were\n                                <div class=\"gender-ratio\">" + gender_ratio.toFixed(1) + "</div> <span class=\"male\">men</span> MPs in " + d.data.year.getFullYear() + ".\n                            </div>";
    ...
    function mouseout(d) {
        if (country_on_screen.indexOf(d.data.country) > -1) {
            d.line.attr("stroke-width", function (d) {
                return d.key == "United Kingdom" ? 0.75 * lineThickness : lineThickness / 4;
...
function to_fifth_slide(current_slide) {
    ...
    switch (current_slide) {
        ...
        case 5:
            ...
            d3.selectAll("#slide6-group, .x-custom-label, .y-axis").style("opacity", 0).style("pointer-events", "none").on("end", function () {
...
function fifth_slide() {
    ...
    // Change credits
    credit_alink.attr("xlink:href", "https://www.theyworkforyou.com/").select("text").transition().text("Data: TheyWorkForYou (raw transcripts)");
    ...
    if (lastTransitioned > 4) {
        ...
        var hist = d3.histogram().domain([-3, 3]).thresholds([-2.5, -1.2, -0.5, 0.5, 1.2, 2.5]);
    ...
    d3.select("body").append("span").attr("class", "slide5-search").append("input").attr("id", "mp-search").attr("type", "text").attr("placeholder", "🔎 Search for your MP by name or constituency");
    ...
    function doSearch() {
        ...
        for (var i = 0; i < results.length; i++) {
            ...
            var mp_string = results[i].full_name + " (" + results[i].party + "-" + results[i].constituency + ")";
    ...
    slide5_xScale = d3.scaleLinear().domain([-450, 150]).range([margin.left, width + margin.left]);
    ...
    yAxis = d3.axisRight(y).ticks(10).tickFormat(function (d) {
        return d * 10 % 1 == 0 ? (d * 100).toFixed(0) + "%" : "";
    });
...
function update_fifth_slide(no_transition, default_selected_topic, from_scroll, drawMedian) {
    ...
    function median_mouseover(nodeData, mousePos) {
        ...
        // Show relevant tooltip info
        tooltip.innerHTML = "\n                            <div class=\"slide5-tooltip\">\n                    <h1 style=\"border-color: " + (nodeData.gender == "female" ? colors["Female"] : colors["Male"]) + ";\">" + (nodeData.gender.toUpperCase() == "female" ? "WOMEN" : "MEN") + "</h1>\n                    The average " + (nodeData.gender.toUpperCase() == "female" ? "WOMAN" : "MAN") + " MP spends <em>" + (nodeData.median * 100).toFixed(1) + "%</em> of " + (nodeData.gender == "male" ? "his" : "her") + " time talking about <em>" + selected_topic + "</em>.\n</div>";
        mouseover_svg.select("circle").datum(nodeData).attr("cx", function (d) {
            return d.x;
        }).attr("cy", function (d) {
            return d.y;
...
function to_sixth_slide(current_slide) {
    ...
    if (lastTransitioned < 5) {
        ...
        x = d3.scaleLinear().range([0, width]).domain([-3, 3]);
        // Redraw axes
        xAxis = d3.axisBottom(x).ticks(6).tickFormat(function (d) {
            return (d * 100).toFixed(0) + "%";
        });
...
function sixth_slide() {
    ...
    if (no_transition == false) {
        ...
        d3.select("#slide6-group").style("pointer-events", "unset");
    }
...
function to_seventh_slide(current_slide) {
    ...
    // Change axes
    x = d3.scaleBand().range([0, width]).domain(parliamentary_candidates_data.map(function (d) {
        return d.year;
    })).paddingInner(0.2);
    xAxis = d3.axisBottom(x);
...
function seventh_slide() {
    ...
    if (no_transition) {
        ...
        d3.select("#slide7-group").selectAll("*").remove().style("pointer-events", "unset");

        slide7Group = zoomedArea.append("g").attr("id", "slide7-group");

        var bar_stack = slide7Group.append("g").selectAll("g").data(d3.stack().keys(["Labour", "Conservative", "Liberal", "Other"])(parliamentary_candidates_data)).enter().append("g").attr("fill", function (d) {
            return colors[d.key];
        });

        var party_bar_group = bar_stack.selectAll("g").data(function (d) {
            d.map(function (i) {
                i["party"] = d.key;return i;
            });return d;
        }).enter().append("g");

        party_bar_group.append("rect").attr("x", function (d) {
            ...
            return d.data.year == "Future" ? "5,5" : "unset";
        }) // If it is the last element, display it differently
        .style("stroke", function (d) {
            return d.data.year == "Future" ? colors["Hover"] : "unset";
        }).style("fill", function (d) {
            return d.data.year == "Future" ? "none" : "unset";
        });

        // Add some text labels for the values
        party_bar_group.append("text").classed("bar-label", true).attr("x", function (d) {
...
function download_data() {
    d3.queue().defer(d3.csv, "data/women_mps.csv", function (d) {
    ...
    }).defer(d3.csv, "data/total_mps_over_time.csv", function (d) {
        var parseDate = d3.timeParse("%Y-%m-%d");
        return {
            year: parseDate(d.Year),
    ...
    d3.queue().defer(d3.csv, "data/mp_base64.csv", function (d) {
        return {
            id: d.id,
            base64: d.base64
        };
    ...
    d3.queue().defer(d3.csv, "data/women_in_govt.csv", function (d) {
    ...
    }).defer(d3.csv, "data/number_women_parliamentary_candidates_over_time.csv", function (d) {
        return {
            year: d.Year,
            Labour: +d.Lab_women,
    ...
    }).defer(d3.csv, "data/photo_attribution.csv").await(function (error, women_in_govt, baked_mp_positions, topic_medians, parliamentary_candidates, photo_attribution) {
        ...
        baked_mp_positions.forEach(function (row) {
            ...
            Object.keys(row).forEach(function (colname) {
                if (colname != "id" & colname != "full_name" & colname != "Constituency" & colname != "Party" & colname != "is_female" & colname.slice(-1) != "y") {
                    ...
                    baked_positions_data.push({
                        ...
                        "search_string": (row["full_name"] + " " + row["Constituency"] + " " + row["Party"] + "-" + row["constituency"]).toLowerCase(),
                        "topic": topic,
                        "x": +row[topic + "_x"],
                        "y": +row[topic + "_y"] / 100
                    });
        ...
        case 1:
            ...
            switch (new_step) {
                ...
                case 1:
                    ...
                    max_mps_path_area.transition().attr("d", max_mps_area).style("fill", colors["Labour"]).style("opacity", 1);
                    mask.transition().attr("d", max_mps_area);
                ...
                case 2:
                    ...
                    max_mps_path.transition().attr("d", max_mps_line);
                    max_mps_area.y1(y(100));
                    max_mps_path_area.transition().attr("d", max_mps_area).style("fill", colors["Conservative"]).style("opacity", 1);
                    mask.transition().attr("d", max_mps_area);
                ...
                case 3:
                    ...
                    max_mps_path.transition().attr("d", max_mps_line);
                    max_mps_area.y1(y(100));
                    max_mps_path_area.transition().attr("d", max_mps_area).style("fill", colors["LD"]).style("opacity", 1);
                    mask.transition().attr("d", max_mps_area);
